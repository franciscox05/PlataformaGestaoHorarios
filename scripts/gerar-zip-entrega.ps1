...
if (-not $JavaHome -or -not (Test-Path (Join-Path $JavaHome "bin\java.exe"))) {
    throw "Indica um JAVA_HOME valido ou instala um JDK 25. Exemplo: .\scripts\gerar-zip-entrega.ps1 -JavaHome 'C:\Users\franc\.jdks\openjdk-25'"
}

$env:JAVA_HOME = $JavaHome
$env:Path = "$JavaHome\bin;$env:Path"

Write-Host "A gerar o ZIP da entrega desktop com JAVA_HOME=$JavaHome" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    throw "A geracao do ZIP falhou."
}

$zipPath = Join-Path $repoRoot "target\PlataformaGestaoHorarios-desktop.zip"

if (-not (Test-Path $zipPath)) {
    throw "O ZIP final nao foi encontrado em $zipPath."
}
