...
public record MesWebOption(int valor, String nome) {

    public static List<MesWebOption> todos() {
        List<MesWebOption> meses = new ArrayList<>();
        for (int mes = 1; mes <= 12; mes++) {
            String nomeMes = Month.of(mes).getDisplayName(TextStyle.FULL, Locale.of("pt", "PT"));
            meses.add(new MesWebOption(mes, capitalizar(nomeMes)));
        }
        return meses;
    }
